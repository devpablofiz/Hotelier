...
public class ClientMain {
            ...
            try (Socket socket = new Socket(tcpHost, tcpPort);
                ...
                while (true) {
                    ...
                    if (input.startsWith("register(")) {
                        ...
                        if (!isLoggedIn) {
                            System.out.println("User needs to be logged in to subscribe for updates");
                            continue;
                        }
                        handleSubscribe(input, listener);
                    } else if (input.startsWith("unsubscribe(")) {
                        if (!isLoggedIn) {
                            System.out.println("User needs to be logged in to unsubscribe for updates");
                            continue;
                        }
    ...
    private static void loadProperties() {
        ...
        try (InputStream input = new FileInputStream("client-config.properties")) {
            if (input == null) {
